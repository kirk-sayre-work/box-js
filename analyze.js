...
function rewrite(code, useException=false) {
    ...
    if (code.length < 2e6) {
        var rvaluePat = /[\n;][^\n^;]*?\([^\n^;]+?\)\s*=[^=^>][^\n^;]+?\r?(?=[;])/g;
        var rvaluePat1 = /([\n;])([^\n^;]*?)\(([^\n^;]+?)\)\s*=([^=^>][^\n^;]+?\r?(?=[;]))/g;
        code = code.toString().replace(rvaluePat1, "$1$2.rvalAssign($3, $4)");
...
`
        ...
        //console.log("!!!! CODE: 11.1 !!!!");
        //console.log(JSON.stringify(tree, null, 2));
        //console.log(escodegen.generate(tree));                
        //console.log("!!!! CODE: 11.1 !!!!");
        
        //if (argv["scrape-string-decodes"]) {
        if (true) {
            lib.verbose("    Modifying return statements to look for decoded URLs...", false);
            traverse(tree, function(key, val) {
                if (!val) return;
                if (val.autogenerated) return;
                if (val.type !== "ReturnStatement") return;
                if (!val.argument) return;
                if (val.argument.type !== "Identifier") return;
                return require("./patches/return.js")(val);
            });
        }
