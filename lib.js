...
function log(tag, text, toFile = true, toStdout = true) {
  ...
  if (!(argv.loglevel in levels)) {
    ...
    log(
      "warn",
      `Log level ${oldLevel} is invalid (valid levels: ${Object.keys(
        levels
      ).join(", ")}), defaulting to "info"`,
      false
    );
  }
  ...
  if (toStdout || argv.loglevel === "debug")
    // Debug level always writes to stdout and file
    console.log(message);
...
function noCasePropObj(obj) {
  var handler = {
    ...
    set: function (target, key, value) {
      if (typeof key == "string") {
        ...
        if (key != uKey && key in target) target[key] = value;
        target[uKey] = value;
    ...
    deleteProperty: function (target, key) {
      if (typeof key == "string") {
        ...
        if (key != uKey && key in target) delete target[key];
        if (uKey in target) delete target[uKey];
      } else delete target[key];
    },
  };
  function checkAtomic(value) {
    if (typeof value == "object") return new noCasePropObj(value); // recursive call only for Objects
    return value;
  }
  ...
  if (typeof obj == "object") {
    ...
    newObj = new Array();
    for (var i = 0; i < obj.length; i++) newObj[i] = checkAtomic(obj[i]);
  }
...
module.exports = {
  ...
  fetchUrl: function (method, url, headers = {}, body) {
    ...
    if (!argv.download) {
      log(
        "info",
        "Returning HTTP 404 (Not found); use --download to actually try to download the payload or --fake-download to fake the download"
      );
      return {
        body: new Buffer(""),
        headers: {},
    ...
    try {
      ...
      log(
        "error",
        `An error occurred while emulating a ${method} request to ${url}.`
      );
      log("error", e);
      throw e;
    }
  ...
  writeFile: function (filename, contents) {
    ...
    if (typeof fileWriteCount[filename] == "undefined")
      fileWriteCount[filename] = 0;
    ...
    if (doLog)
      logIOC(
        ...
        "The script wrote file '" + filename + "'."
      );
  ...
  logResource: function (resourceName, emulatedPath, content) {
    ...
    if (typeof resourceWriteCount[resourceName] == "undefined")
      resourceWriteCount[resourceName] = 0;
    resourceWriteCount[resourceName]++;
    var throttle =
      resourceWriteCount[resourceName] > MAXWRITES * 50 &&
      content.length > MAXBYTES / 5 &&
      throttleWrites;
    if (throttle) {
      if (typeof tooBigFiles[filePath] == "undefined") {
        log(
          ...
          "File '" +
            filePath +
            "' is too big with too many small writes. Not writing."
        );
    ...
    // Don't spam lots of file write info to same file.
    if (typeof fileWriteCount[filePath] == "undefined")
      fileWriteCount[filePath] = 0;
    ...
    if (doLog) {
      ...
      filetype = child_process
        .execSync("file " + simpleStringify(filePath))
        .toString("utf8");
    ...
    if (doLog && /executable/.test(filetype)) {
      ...
      fs.writeFileSync(
        path.join(directory, "active_urls.json"),
        simpleStringify(activeUrls, null, "\t")
      );
    }
    ...
    if (doLog) {
      ...
      fs.writeFileSync(
        ...
        simpleStringify(resources, null, "\t")
      );
