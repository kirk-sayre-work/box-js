...
const flags = JSON.parse(
  fs.readFileSync(path.join(__dirname, "flags.json"), "utf8")
);
...
const allFlags = [...flags.run, ...flags.export].reduce((set, item) => {
  // Make unique
  if (!set.some((it) => it.name === item.name)) return set.concat(item);
  return set;
}, []);
const argv = commandLineArgs(allFlags, { partial: true });
if (argv._unknown != null && argv._unknown.some(isFlag)) {
  throw new Error(`Unknown arguments: ${argv._unknown.filter(isFlag)}`);
}
